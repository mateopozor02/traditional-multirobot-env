...
def send_goal(position_x, position_y, orientation_z, orientation_w, marker_pub):
    ...
    goal_publisher = rospy.Publisher('/tb3_0/move_base_simple/goal', PoseStamped, queue_size=10)

    # Create a subscriber for the estimated position topic of the robot
    rospy.Subscriber('/tb3_0/amcl_pose', PoseWithCovarianceStamped, amcl_pose_callback)
...
def main():
    ...
    marker_pub = rospy.Publisher('/tb3_0/trajectory_marker', Marker, queue_size=10)
    ...
    try:
        ...
        #send_goal(14, 7.5, 0.0, 0.0, marker_pub)
        #rospy.sleep(1)  # Wait 5 seconds after reaching the first position

        send_goal(16.5, 3.7, 0.0, 0.0, marker_pub)
        rospy.sleep(1)  # Wait 5 seconds after reaching the first position
