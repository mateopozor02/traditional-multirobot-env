...
def send_goal(position_x, position_y, orientation_z, orientation_w, marker_pub):
    ...
    goal_publisher = rospy.Publisher('/tb3_1/move_base_simple/goal', PoseStamped, queue_size=10)
...
def publish_path(marker_pub):
    ...
    marker.scale.y = 0.1
    marker.scale.z = 0.1
    marker.color = ColorRGBA(0.5, 0.0, 0.5, 1.0)  # Purple color for the points
...
def main():
    ...
    marker_pub = rospy.Publisher('/tb3_1/trajectory_marker', Marker, queue_size=10)
    ...
    try:
        ...
        #send_goal(14, 8.0, 0.0, 0.0, marker_pub)
        #rospy.sleep(1)  # Wait 5 seconds after reaching the first position

        send_goal(17, 3.7, 0.0, 0.0, marker_pub)
        rospy.sleep(1)  # Wait 5 seconds after reaching the first position
